...
var (
	...
	iconLeftClickMu       sync.RWMutex
	iconLeftClickHandlers []func()
)
...
// OnLeftClick registers a handler for left-click on the tray icon (Windows).
// When at least one handler is registered, left-click does not open the menu.
func OnLeftClick(fn func()) {
	if fn == nil {
		return
	}
	iconLeftClickMu.Lock()
	iconLeftClickHandlers = append(iconLeftClickHandlers, fn)
	iconLeftClickMu.Unlock()
}

func systrayIconLeftClicked() {
	iconLeftClickMu.RLock()
	handlers := append([]func(){}, iconLeftClickHandlers...)
	iconLeftClickMu.RUnlock()
	if len(handlers) > 0 {
		// Never call Wails/runtime from wndProc — it blocks the tray message loop.
		go func() {
